...
2. 增加了 `examples/libero/convert.py` 文件，在原来的 `examples/libero/convert_mydata.py` 基础上，实现对于错误格式的episode的跳过，并在最后统计处理总处理条数和失败条数。
3. 增加 `src/openpi/policies/ur11_policy.py` 文件，修改 LiberoInputs 类里对 wrist_img 的处理。
4. 对原来的 `config.py` 文件的更改
   - 增加 LeRobotUR11DataConfig 类，该类是在原来 LeRobotLiberoDataConfig 类的基础上，调用 `ur11_policy.` 。
   - 在 _CONFIGS 里增加一个 `pi0_libero_low_mem_finetune_shenghe` 微调，repo_id 为 lerobot 数据的地址 `shenghe/libero` ，其他内容和 `pi0_libero_low_mem_finetune` 微调保持一致。
